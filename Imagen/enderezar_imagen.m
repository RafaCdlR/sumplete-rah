...
function [I, angulo] = enderezar_imagen(I) 
    ...
    for k = 1:length(lines)
        ...
        angulo_linea = atan2d(delta_x, delta_y);

        % Verificar si la línea es "vertical" (ángulo cercano a ±90 grados)
        if abs(angulo_linea) > 45 % Se considera vertical si el ángulo es mayor a 45 grados
            % Actualizar si la longitud es mayor
            if len > max_len
                max_len = len;
                linea_vertical = lines(k);
            end
        end
    end
